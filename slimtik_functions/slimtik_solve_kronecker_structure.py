...
def solve(Z, C, M, W, sumLambda,
    ...
    # U, S, _ = torch.svd(M.to(dtype)+torch.eye(M.shape[0], M.shape[1]))
...
def choose_Lambda_candidates(sumLambda, upper_bound, lower_bound, dtype=torch.float64, device='cpu'):
    ...
    Lambda1 = torch.logspace(math.log10(eps), math.log10(upper_bound), 100, device=device)
    ...
    if n_low <= 0:
        ...
        Lambda2 = -torch.logspace(math.log10(n_low), math.log10(eps), 100, device=device)
