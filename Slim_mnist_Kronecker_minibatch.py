...
class MyCNN(nn.Module):
    ...
    def forward(self, x, y=None, separate=False):
        ...
        if separate:
            with torch.no_grad():
                ...
                # solve for W and b
                self.solve(Z, y.T.to(self.device), n_calTk)
    ...
    def solve(self, Z, C, n_calTk, dtype=torch.float32):
        ...
        W, info = tiksolvevec.solve(beta * Z, beta * C, beta * M, self.W, self.sumLambda, Lambda=self.Lambda,
                                    dtype=dtype, device=self.device, opt_method=self.opt_method,
